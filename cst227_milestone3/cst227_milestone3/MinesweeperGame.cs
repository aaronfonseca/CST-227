...
namespace cst227_milestone3
{
	class MinesweeperGame: Grid, IPlayable
	{
		...
		{ 
			...
			bool cellVal = game.square[i, j].getLive();
			if (cellVal == false)
			{
				// leftNeighbor
				if (i > 0)
				{
					// Get Neighbors
					double val = game.square[i - 1, j].getNeighbors();
					bool visited = game.square[i - 1, j].getVisited();

					// if neighbor 0
					if ((Convert.ToInt32(val) == 0) && (!visited))
					{
						// turn neighbor to visited
						game.square[i - 1, j].setVisited(true);
						revealZeros(i - 1, j);
					}
				...
				// rightNeighbor
				if (i < game.square.GetLength(0) - 1)
				{
					// Get Neighbors
					double val = game.square[i + 1, j].getNeighbors();
					bool visited = game.square[i + 1, j].getVisited();
				...
				{
					// Get Neighbors
					double val = game.square[i, j - 1].getNeighbors();
					bool visited = game.square[i, j - 1].getVisited();

					// if neighbor 0
					if ((Convert.ToInt32(val) == 0) && (!visited))
					{
						// turn neighbor to visited
						game.square[i, j - 1].setVisited(true);
						revealZeros(i, j - 1);
					}
					else if ((Convert.ToInt32(val) < 9) && (!visited))
					{
				...
				// bottomNeighbor
				if (j < game.square.GetLength(0) - 1)
				{
					double val = game.square[i, j + 1].getNeighbors();
					bool visited = game.square[i, j + 1].getVisited();
		...
		public void playGame() 
		{
			...
			while (gameRunning)
			{
				...
				if ((game.square[y, x].getLive()) && (Convert.ToInt32(game.square[y, x].getNeighbors()) == 9))
				{
					...
					// Set cell to visited
					game.square[y, x].setVisited(true);
					this.revealZeros(y, x);
					Console.Clear();
